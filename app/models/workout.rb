class Workout < ActiveRecord::Base
  ...
  def self.clone(user, base_workout_id)
    clone_task_id_hash = Hash.new
    clone_set_id_hash = Hash.new
    ...
    base_workout.task.each do |base_task|
      ...
      clone_task_id_hash[base_task.id] = clone_task.id
      base_task.exercise_set.each do | base_set |
        clone_set = clone_task.exercise_set.create(rep_count: base_set.rep_count, rep_value: base_set.rep_value, rep_unit: base_set.rep_unit, state: base_set.state)
        clone_set_id_hash[base_set.id] = clone_set.id
      end
    end
    return clone_workout.id, clone_task_id_hash, clone_set_id_hash
  end
